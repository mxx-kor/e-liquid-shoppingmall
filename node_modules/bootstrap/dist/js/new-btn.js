...
function addCart() {
    if (document.getElementById('finalItem1') == null && document.getElementById('finalItem2') == null) {
        ...
        if (document.getElementById('addedItem') == null) {
            ...
            LIQUID_quantity = document.getElementById("finalItem2").innerText;
            // COOLING_quantity = document.getElementById("finalItem1").innerText.slice(0, -1);
            payCheck = resultCheck.innerText;
            sessionStorage.setItem(LIQUID_NAME, document.getElementById('liquid-title').innerText);
            sessionStorage.setItem(LIQUID_THUMBNAIL, document.getElementById('liquid-thumbnail').src);
            sessionStorage.setItem(LIQUID, LIQUID_quantity);
            if(document.getElementById("finalItem1") != null) {
                COOLING_quantity = document.getElementById("finalItem1").innerText;
                sessionStorage.setItem(COOLING, COOLING_quantity);
            }
            sessionStorage.setItem(TOTAL, payCheck);
        }else {
            alert('이미 추가된 상품입니다.');
        }        
    }
...
// json 에서 필요한거 - 이미지 소스, 이름, 내용물, 개수 
//구매하기 버튼 추가상품만 주문 거부랑 모달하나 띄울까?
function saveCart () {
    localStorage.setItem(LIQUID, JSON.stringify(liquid));
}